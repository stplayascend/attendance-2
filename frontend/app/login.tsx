  ...
  const submit = async () => {
    ...
    try {
      const { data } = await api.post("/auth/login", {
        identifier: identifier.trim(), password: pw,
      });
      await setAuth(data.token, data.user);
      if (data.user.role === "admin") router.replace("/admin/dashboard");
      else if (data.user.role === "teacher") router.replace("/teacher/dashboard");
      else {
  ...
  return (
    <FormScreen>
      ...
      <Text style={[typography.bodyMuted, { marginTop: 6 }]}>
        Use your USN (student) or Employee ID (teacher).
      </Text>
      ...
      <View style={s.linkRow}>
        <TouchableOpacity testID="link-student-register" onPress={() => router.push("/student/register")}>
          <Text style={s.link}>
            <Feather name="user-plus" size={14} color={colors.brand} /> Student Sign Up
          </Text>
        </TouchableOpacity>
        <TouchableOpacity testID="link-teacher-register" onPress={() => router.push("/teacher-register")}>
          <Text style={s.link}>
            <Feather name="briefcase" size={14} color={colors.brand} /> Register as Teacher
          </Text>
        </TouchableOpacity>
      </View>
    </FormScreen>
...
const s = StyleSheet.create({
  ...
  hero: { width: "75%", height: 180 },
  linkRow: {
    flexDirection: "row", justifyContent: "space-between",
    marginTop: spacing.xl, paddingHorizontal: 8,
  },
